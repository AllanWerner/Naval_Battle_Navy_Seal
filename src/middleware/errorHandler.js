...
// Valide le champ "title". requireTitle=true pour une création (POST),
// false pour une mise à jour partielle (PUT) où le title est optionnel.
function validateTitle(title, requireTitle, next) {
  if (title === undefined) {
    if (requireTitle) {
    ...
    // Non fourni sur une mise à jour partielle : on ne touche pas au champ
    return undefined;
  }
...
// Factory de middleware de validation pour les requêtes POST (création,
// title obligatoire) et PUT (mise à jour partielle, title optionnel).
function makeTaskValidator(requireTitle) {
  return (req, res, next) => {
    try {
      ...
      // === 3. VALIDATION DU TITLE ===
      const validatedTitle = validateTitle(title, requireTitle, next);
      if (validatedTitle === null) {
        // Une erreur a déjà été transmise à next() par validateTitle
        return;
      }
      ...
      if (status !== undefined && status !== null) {
        ...
        if (!VALID_STATUSES.includes(status)) {
          const error = new Error(`Status invalide. Valeurs acceptées: ${VALID_STATUSES.join(', ')}`);
          error.status = 400;
          error.details = { field: 'status', validValues: VALID_STATUSES };
          return next(error);
        }
      }
      ...
      if (priority !== undefined && priority !== null) {
        ...
        if (!VALID_PRIORITIES.includes(priority)) {
          const error = new Error(`Priorité invalide. Valeurs acceptées: ${VALID_PRIORITIES.join(', ')}`);
          error.status = 400;
          error.details = { field: 'priority', validValues: VALID_PRIORITIES };
          return next(error);
        }
      }
      ...
      if (dueDate !== undefined && dueDate !== null) {
        ...
        // Vérifier que la date n'est pas dans le passé (uniquement à la création :
        // sur une mise à jour on peut vouloir corriger une tâche déjà en retard)
        if (requireTitle && date < new Date()) {
...
// Création : le titre est obligatoire
const validateTaskCreate = makeTaskValidator(true);
// Mise à jour partielle : le titre est optionnel (seuls les champs
// fournis sont validés puis mis à jour, cf. taskController.updateTask)
const validateTaskUpdate = makeTaskValidator(false);
